 ...
 *
 */
export async function checkIfNewTransaction(orderModel: any, paypalTransactionId: string) {
  try {
...
export async function verifyPayPalPayment(paypalTransactionId :string) {
  try {
    ...
    if (!paypalResponse.ok) {
      const errorResponse = await paypalResponse.json();
      console.error('Failed to verify payment. PayPal API returned:', errorResponse);
      ...
      throw new Error(`Failed to verify payment. PayPal API returned status ${paypalResponse.status}`);
    }
    ...
    console.error('Error in verifyPayPalPayment:', error);
